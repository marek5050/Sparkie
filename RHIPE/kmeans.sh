...
kmeans_test <- function(data_size, num_clusters, num_iterations, directory) {
   ...
   input<-list()
   for(i in 1:10) input<-c(input,list(list(rnorm(1, sd=10),rnorm(1,sd=10))));
  rhdel(directory)
   ...
   # }
    ...
    job <- rhwatch(
        ...
        output="hdfs:///user/dotcz12/outkmeans",
        mapred=NULL,
        jobname="kmeans",
        verbose=TRUE) 
        # rhex(job)  
      # C = rhread("hdfs:///user/dotcz12/outkmeans")
    # }
    # C
}

kmeans_test(1,5,5, 'hdfs:///user/dotcz12/kmeans')
